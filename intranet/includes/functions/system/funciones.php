<?php
	class Funciones{
		...
		public static function obtenerUrl($url){
			$url_validar = explode("/", $url);
			$contador = count($url_validar);
			$url_final = $url_validar[$contador-1];
			$condicion = strpos($url_final, '?');

			if($condicion != null){
				$url_final2 = explode("?", $url_final);
				$url_final = $url_final2[0];
			}
