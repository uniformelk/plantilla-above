...
if(count($validatabla)==1 && $id_proyecto!= 30){
//si tiene modalidad se consulta
    if(isset($base_datos)){
    
    $conexionAlterna->conectarAlterno($servidor,$usuario,$password,$base_datos);
    $resultadoRe = $conexionAlterna->ejecutarConsulta("SELECT modalidad, COUNT(*) AS total FROM ".$tabla." GROUP BY modalidad ORDER BY total DESC;");
}?>
...
?>
    ...
    <div class="card shadow mb-4 w-100 oculto  mt-3" id="datos">
...
<div class="row">
    <?php 
        ...
        foreach($validatabla as $fila){
            ...
            if(isset($base_datos)){
                $conexionAlterna->conectarAlterno($servidor,$usuario,$password,$base_datos);
                $resultadoRe = $conexionAlterna->ejecutarConsulta("SELECT COUNT(*) AS total FROM ".$fila.";");
            }
